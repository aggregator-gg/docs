...
{"transaction_id":"tx_bet_001","type":"bet","session_id":"...","amount":500,"currency":"EUR",...}
```
...
2. HMAC-SHA256 with your operator's `callback_secret` (set in the cabinet under **API Keys**)
...
| Using `sk_live_*` API key as the secret | Always fails — wrong secret | The `callback_secret` is separate; find it in cabinet → **API Keys** → **Callback secret** |
