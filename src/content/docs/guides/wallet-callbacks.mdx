...
:::note[Units: `amount` and `balance` are the same denomination]
Both the request `amount` (this payload) and the response `balance` (your reply) are **integers in minor units** — never decimal strings. You can apply `amount` to your stored balance directly with **no unit conversion**.

The minor unit is set by the ISO-4217 **currency**, not by you:

| Currency type | Examples | Conversion | `amount` for 1.50 of currency |
|---------------|----------|------------|-------------------------------|
| 2-decimal | EUR, USD, GBP | major ×100 | `150` (= €1.50) |
| 0-decimal | JPY, KRW | 1:1 | `278` (= ¥278) |
| 3-decimal | KWD, BHD, OMR | major ×1000 | `1500` (= 1.500 KWD) |

Use integer arithmetic (or a decimal type) throughout — never floating point.
:::
...
| Field | Type | Description |
|-------|------|-------------|
| `balance` | `integer` | Player's balance **after** applying the transaction, in minor units (cents) — the same unit as the request `amount`. |
