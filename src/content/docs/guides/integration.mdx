...
**Prerequisites**:
- An operator account in The Aggregator cabinet ([register here](https://aggregator.gg/register))
...
1. Register as an operator at [aggregator.gg/register](https://aggregator.gg/register).
2. Complete the KYB (Know Your Business) verification in the operator cabinet.
3. Navigate to **Settings > API Keys** in your cabinet.
4. Use **Get readiness** to confirm the live promotion checklist.
5. Use **Promote to live** to create the production key when you are ready.
6. You receive two API keys:

| Key | Format | Purpose |
|-----|--------|---------|
| **Sandbox** | `sk_test_*` | Integration development and testing. No real money, no real providers. |
| **Production** | `sk_live_*` | Live player traffic. Only use after completing the go-live checklist. |

7. Copy your **callback secret** from the same page. You use this to verify callback signatures.

> **Security**: Store your API keys and callback secret in environment variables or a secrets manager. Never commit them to source control.

---

## Step 2: Explore the Game Catalog

Use the sandbox key to browse available games.
...
const aggregator = new Aggregator({ apiKey: "sk_test_xxx" });
...
You can filter by provider, game type, volatility, RTP range, features, and full-text search. See the [full API reference](machine-api-reference.md) for all query parameters.
...
For full callback documentation, see the [Wallet Callback Guide](wallet-callback-guide.md).

---

## Step 5: Test Your Integration

Use your `sk_test_*` sandbox key to test the full flow end-to-end.
...
- [ ] **Demo session** -- Create a demo session and verify free-play mode works
...
You can inspect all sandbox transactions through the API:
...
Once your sandbox integration passes all tests, you are ready to switch to production.

### Production checklist

**API and authentication:**
- [ ] Replace `sk_test_*` with your `sk_live_*` API key in your production environment
- [ ] Store the production API key and callback secret in a secrets manager (not in code)
- [ ] Verify your production callback URL is configured in the operator cabinet
...
- [Full API Reference](machine-api-reference.md) -- Complete endpoint documentation with request/response schemas
- [Wallet Callback Guide](wallet-callback-guide.md) -- Detailed callback specification, retry behavior, and edge cases
- [@aggregator/sdk README](../../packages/sdk/README.md) -- TypeScript SDK installation and usage