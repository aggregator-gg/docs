...
- `game_url` is single-use. Create a new session each time the player needs to launch the game.
...
- `amount` is a positive **integer in minor units** (e.g. `500` = 5 EUR) — the **same unit** as the response `balance` below. The minor unit is determined by the ISO-4217 currency: 2-decimal currencies (EUR/USD) use 1/100 (`500` = €5.00), 0-decimal currencies (JPY/KRW) are 1:1 (`278` = ¥278), and 3-decimal currencies (KWD/BHD/OMR) use 1/1000.
...
`balance` is the player's new balance in **minor units** (cents — e.g. `9500` = 95.00 EUR). This is the **same denomination** as the request `amount` (an integer in minor units, `500` = €5.00), so you can apply `amount` to `balance` directly with no unit conversion. `currency` must match the request.
...
app.post("/callbacks/wallet", async (req, res) => {
  ...
  // `amount` is an integer in minor units (cents) — the same unit as `balance`,
  // so it can be applied to the balance directly with no conversion.
